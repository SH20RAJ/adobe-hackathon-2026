  ...
  const handleRunAudit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Fallback to sample or synthesized report if offline
            ...
            <Card className="border-border bg-card/60 relative overflow-hidden">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={report.summary.acpi_score >= 75 ? "success" : "warning"}>
                    {report.summary.acpi_score >= 75 ? "High Citability" : "Optimization Needed"}
                  </Badge>
                </div>
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="text-4xl sm:text-5xl font-extrabold tracking-tight text-foreground font-mono">
                    {report.summary.acpi_score.toFixed(1)}
                  </span>
            ...
            <Card className="border-border bg-card/60 relative overflow-hidden">
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={report.summary.crs_score >= 75 ? "success" : "warning"}>
                    {report.summary.crs_score >= 75 ? "Low Bounce Risk" : "Friction Detected"}
                  </Badge>
                </div>
                <div className="mt-3 flex items-baseline gap-2">
                  <span className="text-4xl sm:text-5xl font-extrabold tracking-tight text-foreground font-mono">
                    {report.summary.crs_score.toFixed(1)}
                  </span>
                  <span className="text-sm font-semibold text-muted-foreground">/ 100</span>
                </div>
                <p className="mt-2 text-xs text-muted-foreground leading-relaxed">
            ...
            <Card className="border-border bg-card/60">
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-mono text-muted-foreground">{report.summary.total_findings} items</span>
                </div>
                <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
                  <div className="p-2.5 rounded-xl bg-destructive/10 border border-destructive/20 flex items-center justify-between">
                    ...
                    <span className="font-bold text-destructive font-mono">{report.summary.critical}</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-between">
                    ...
                    <span className="font-bold text-amber-400 font-mono">{report.summary.high}</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-between">
                    ...
                    <span className="font-bold text-primary font-mono">{report.summary.medium}</span>
                  </div>
                  <div className="p-2.5 rounded-xl bg-muted/60 border border-border/50 flex items-center justify-between">
                    ...
                    <span className="font-bold text-foreground font-mono">{report.summary.low}</span>
                  </div>
                </div>
          ...
          <Card className="border-border bg-card/50">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {[
                  { label: "Crawlability & AI Bots", score: report.summary.component_scores.crawlability, weight: "30% ACPI" },
                  { label: "Hydration & DOM Gap", score: report.summary.component_scores.renderability, weight: "15% ACPI" },
                  { label: "Entity Clarity & sameAs", score: report.summary.component_scores.entity_clarity, weight: "20% ACPI" },
                  { label: "AEO Quotability Density", score: report.summary.component_scores.quotability, weight: "20% ACPI" },
                  { label: "Freshness & E-E-A-T Trust", score: report.summary.component_scores.trust_freshness, weight: "15% ACPI" },
                  { label: "Value Prop & Readability", score: report.summary.component_scores.orientation, weight: "55% CRS" },
                ].map((c) => (
                  <div key={c.label} className="p-3 rounded-xl bg-background/50 border border-border/40 space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-medium text-foreground">{c.label}</span>
                      <span className="font-mono font-bold text-primary">{c.score.toFixed(0)}%</span>
                    </div>
                  ...
                  <CardContent className="p-5 space-y-3">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-2 text-xs font-mono">
                        ...
                        <span className={f.score_impact < 0 ? "text-destructive font-bold" : "text-emerald-400 font-bold"}>
                          {f.score_impact > 0 ? `+${f.score_impact}` : f.score_impact} pts
                        </span>
                      </div>
                    </div>
                    ...
                    {f.evidence?.snippet && (
                      <div className="p-2.5 rounded-lg bg-background/80 border border-border/50 text-xs font-mono text-muted-foreground overflow-x-auto">
                        <div className="text-[10px] text-muted-foreground/80 mb-1 uppercase font-sans font-semibold">Detected Evidence:</div>
                        <code>{f.evidence.snippet}</code>
                      </div>
                    )}
                    ...
                    {f.suggested_action?.code_patch && (
                      <div className="space-y-1.5 pt-1">
                        <div className="flex items-center justify-between">
                          <span className="text-xs font-semibold text-primary">{f.suggested_action.title}</span>
                          <button
                            type="button"
                            onClick={() => copyCode(f.id, f.suggested_action.code_patch!)}
                        ...
                        <p className="text-[11px] text-muted-foreground">{f.suggested_action.explanation}</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
